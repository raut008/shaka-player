...
shaka.net.HttpFetchPlugin = class {
  ...
  static async request_(uri, requestType, init, abortStatus, progressUpdated,
    ...
    // shaka.log.info('shakaplayer:http_fetch_plugin:request_ ', uri);
    ...
    try {
      ...
      let numRedirects = 0;
      while (response.status == HTTP_MOVED_PERMANENTLY ||
             response.status == HTTP_REDIRECT_FOUND ||
             response.status == HTTP_TEMPORARY_REDIRECT ||
             response.status == HTTP_PERMANENT_REDIRECT) {
        if (numRedirects > HTTP_MAX_REDIRECTS) {
          shaka.log.v1('Max redirects reached. Probably a redirect loop');
          break;
        }

        const redirectUri = response.headers.map['location'];
        if (redirectUri !== undefined && redirectUri !== null) {
          response = await fetch(redirectUri, init);
          shaka.log.v1('Redirect Response Status: ', response.status);
          numRedirects++;
        } else {
          shaka.log.error('Cannot redirect. Location header is unavailable');
          break;
        }
      }
      ...
      if (init.method != 'HEAD') {
        ...
        // AMZN_CHANGE_BEGIN
        shaka.log.v2('shakaplayer:http:content len = ', contentLength);
