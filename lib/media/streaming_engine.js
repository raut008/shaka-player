...
shaka.media.StreamingEngine = class {
   ...
   * @private
   */
  async aes128Decrypt_(rawResult, aes128Key, position) {
    const key = aes128Key;
    if (!key.cryptoKey) {
      ...
      if (reference.initSegmentReference) {
        ...
        const append = async () => {
          try {
            ...
            shaka.log.debug('shakaplayer:engine:appendBuffer initSegment done');
          } catch (error) {
  ...
  async append_(mediaState, presentationTime, stream, reference, segment,
      isChunkedData = false) {
    shaka.log.debug('shaka:engine:append_ presentationTime:', presentationTime);
    const logPrefix = shaka.media.StreamingEngine.logPrefix_(mediaState);
    ...
    shaka.log.v2('shakaplayer:engine:append_:appendBuffer mediaSegment type=',
        mediaState.type, ', segment=',
        stream.segmentIndex !== null ? stream.segmentIndex : 'null');
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    ...
    if (reference.aes128Key) {
      result = await this.aes128Decrypt_(result, reference.aes128Key, position);
    }
    return result;
  }
