...
shaka.util.XmlUtils = class {
  ...
  static findChildren(elem, name) {
    ...
    for (const child of elem.childNodes) {
      if (child.nodeName == name) {  // AMZN_CHANGE_ONELINE
        found.push(child);
      }
    }
    return found;
  ...
  static findChildrenNS(elem, ns, name) {
    const found = [];
    for (const child of elem.childNodes) {
      if (child.localName == name &&              // AMZN_CHANGE_ONELINE
  ...
  static parseXmlString(xmlString, expectedRootElemName) {
    ...
    // AMZN_CHANGE_BEGIN
    /*
    // NodeIterator and NodeFilter do not exist
    // in XXXXScript. Created JIRA to implement
    // polyfills. PROJ****-95196
